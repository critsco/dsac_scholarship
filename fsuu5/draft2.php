...
<body class="bg-light">
    ...
    <div class="container d-flex align-items-center justify-content-between">
      ...
      <nav id="navbar" class="navbar">
            <ul>
                ...
                <li><a class="nav-link scrollto active" href="draft2.php">Drafts</a>
  ...
  <main id="main">
    ...
    $sql_draft = "SELECT * FROM scholarship WHERE status = 'Draft' ORDER BY id DESC";
...
while ($row_draft = $result_draft->fetch_assoc()) {
    echo "<tr>
            <td style='display:none;'>{$row_draft['post_date']}</td>
            <td>{$row_draft['type']}</td>
            <td>{$row_draft['provider']}</td>
            <td>{$row_draft['scholarship']}</td>
            <td>{$row_draft['grade_level']}</td>
            <td>{$row_draft['benefits']}</td>
            <td style='text-align: center;'>" . date('Y-m-d', strtotime($row_draft['start_date'])) . "</td>
            <td style='text-align: center;'>" . date('Y-m-d', strtotime($row_draft['end_date'])) . "</td>
            <td class='text-center'><a href='{$row_draft['file_attachment']}' target='_blank' class='btn btn-primary'>Details</a></td>
            <td class='text-center'>
            <form method='post' style='display:inline;'>
                <input type='hidden' name='id' value='" . $row_draft['id'] . "'>
                <button type='submit' name='post_button' class='btn btn-outline-primary'>Post</button>
            </form> <br> <br>
            
            <form action='edit_draft2.php' method='get' style='display:inline; margin-left: 5px;'>
                <input type='hidden' name='id' value='" . $row_draft['id'] . "'>
                <button type='submit' class='btn btn-outline-warning'>Edit</button>
            </form> <br> <br>
            
            <form action='delete_draft2.php' method='get' style='display:inline; margin-left: 5px;'>
                <input type='hidden' name='id' value='" . $row_draft['id'] . "'>
